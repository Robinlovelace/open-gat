...
<!-- transport demand -->
modelling, routing and route network analysis.
<!-- to estimate the impacts of change, -->
...
4. Classify and the projects based on their main user interface, resulting in the top 10 open source tools for geographic analysis in transport planning shown in Table \@ref(tab:open-tools) (see the appendix for a complete table of results).
...
To identify open source projects on GitHub an alternative approach was taken due to the limited capabilities of GitHub's [advanced search page](https://github.com/search/advanced):
An 'snowball' method, analogous to that used by @grabowicz_social_2012 in the context of social media, was used whereby known projects were identified and the 'topic' descriptions they used were searched for to identify additional projects and search terms.
...
- The search term [`transport planning`](https://github.com/search?q=transport+planning&type=Repositories&l=&l=) identified the stplanr project.
- One of the 'topics' in the stplanr repository was was the broader term `transport`, which was used to identify the SUMO projct
- The SUMO project also had the term 'simulation', leading to the discovery of the abstreet project
...
library(kableExtra)
# open_tools_all = readODS::read_ods("open-tools.ods")
# open_tools = open_tools_all %>%
#   select(tool_name, interface, type, licence, langage, website, stars, citations = n_citations, maintained, reference) %>%
#   mutate(
#     # tool_name = paste0("[", tool_name, "](", website, ")"),
#     # tool_name = cell_spec(tool_name, format = "latex", link = website),
#     stars = as.numeric(stars),
#     citations = as.numeric(citations)
#     ) %>% 
#   filter(maintained == 1) %>%
#   # mutate(tool = paste0("\\href{http://", website, "}{", tool_name, "}")) %>%
#   mutate(tool = tool_name) %>%
#   relocate(tool) %>% 
#   arrange(interface, desc(stars)) %>%  
#   select(-maintained, -tool_name)
# readr::write_csv(open_tools, "open_tools.csv")
...
open_tools %>% 
  ...
  # pack_rows(group_label = "WUI", start_row = 19, end_row = 23) %>%
...
7 of those were routing engines, which were excluded for reasons outlined in the previous section, resulting in 23 projects outlined in Table \@ref(tab:open-tools)).
Although these tools could also be classified into different 'ecosystems', such as R packages, Python packages and QGIS plugins, it is clear that some tools do not fit into such prescribed boxes.
We therefore describe the tools in order of their primary user interface, in chronological order of the interface's development (CLIs predate GUIs which predate WUIs).
...
It should be clear that the 'Type' and 'Language' values shown in Table \@ref(tab:open-tools) are to some extent fuzzy: open source software is by nature modular and flexible, meaning that the same piece of code can take multiple different forms and the same method can be implemented in multiple languages.
...
The point is that the *most prominent* category into which each project seemed to fall, based on the README, was used.
...
```{r, message=FALSE}
# open_tools = readr::read_csv("open-tools-for-transport.csv") 
# # open_tools$`Search terms`
# open_tools_min = open_tools %>% 
#   select(-matches("website")) %>% 
#   mutate_all(~case_when(is.na(.) ~ "", TRUE ~ as.character(.) ))
# 
# 
#   # select(-matches("data|routing|vis|website"))
# open_tools_min %>% knitr::kable("latex", booktabs = T) %>%
...
Both projects are traffic microsimulation models (with meso and in the cast of MATSim macroscopic capabilities) that simulate individual vehicles at high spatial and temporal resolution.
Neither project focus on geographic analysis, but they both rely on geographic inputs (detailed road geometries) and produce geographic outputs.
...
Python packages **UrbanAccess**, **spaghetti** and **scikit-mobility** have broader (and to some extent overlapping) remits, aiming to support a range of transport planning objectives.
**UrbanAccess** is a Python package for analysing geographic transport network data, with a focus on accessibility.
The paper describing the tool has more of a focus on applied transport planning than other Python tools, highlighting the potential for the tool to assist metropolitan planning organizations (MPOs) to prioritise investments that cost-effectively increase accessibility for those most in need [@blanchard_urbanaccess_2017].
...
The QGIS plugins for transport planning are even more explicitly focussed on geographic analysis for transport planning.
...
The **Networks** plugin uses an interface to external software developed by the same author to enable multi-modal routing and GTFS data import. 
...
Many of the tools presented in the previous section fit into one or more software 'ecosystems', by which we mean communities of software developers linked by an overarching organisation, technology or language.
Software is not developed in isolation but in a social context [@dhir_adoption_2017], three of these ecosystems from which several open source tools for geographic analysis in transport planning have  --- R, Python and QGIS --- are described below.
...
R, Python and QGIS ecosystems were selected due to their maturity, wide uptake in industry and academia and, above all, for their mature support for geographic analysis.
...
In the context of now policy drivers --- including the obesity crisis, air pollution concerns and the 'climate emergency' that has been declared by some city authorities --- many transport planners have been tasked with new sustainable transport targets.
These include discouraging energy intensive modes, particularly car use, and enabling more walking and cycling [@hickman_transitions_2011] .
Furthermore, in the age of evidence-based policy, open data and citizen science, there is an onus on practitioners to provide solutions that are transparent, accessible and, increasingly, participatory  [@banister_sustainable_2008;@peters_citizen_2020].
...
A shift to open source planning tools in general, and open source tools for local planning in particular, can help help tackle wider problems, including the 'crisis of participatory planning' [@legacy_there_2016], feelings of dis-empowerment due to lack of opportunities to engage in democratic processes --- which some have blamed for the election of populist leaders and the UK Brexit vote [@monbiot_out_2017] --- and low levels of data and software literacy [@christozov_data_2017].
A key feature of geographically localised transport plans is that they encourage participation by local residents, who have access to information about their local area that goes beyond the datasets beyond the reach of remote planners.
And a key feature of open software is that anyone can use it, encouraging citizen engagement.

Partly in response to these pressures and challenges, a number of open source transport planning tools have emerged, notably MATSim, SUMO and sDNA, as outlined in Section 4.
...
As the FOSS philosophy described in Section \@ref(open-source-tools-for-transport-planning) emphasises, open source software by its very nature is collaborative, innovative and evolving [@gancarz_linux_2003], allowing it to out-compete and eventually dominate in sectors from machine learning to web development.
