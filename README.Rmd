...
Geographic analysis has a huge amount to offer transport planning and, more specifically, to transport planning tools.
...
However, many of the 'tools of the trade' for geographic analysis in transport planning have historically been proprietary
<!-- or difficult to integrate into the wider planning process or both. -->
In this context, the aim of this paper is to describe emerging open source options for geographic analysis in transport planning.
Solutions are reviewed with reference to academic and practitioner-led literature, with explicit coverage of demand-side data, data processing and geographical data generation (including transport routing and route network generation).
A key finding from the literature is that a growing number of solutions exist that can help tackle these issues, ranging from single functions dedicated to a particular geographic transport planning task to large software projects.
Prominent options include dedicated transport modelling software products such as MATsim and SUMO, in addition to more general programs such as QGIS, as well as open source languages such as R and Python which are used as the foundation of a growing number of 'packages' for transport planning.
...
Overall we conclude that there has never been a better time for transport planners to 'shift gear' and adopt open source solutions end-to-end in their everyday practice.
<!-- otwithstanding institutional barriers,  -->
...
Successful transport plans are therefore a combination of geographically specific recommendations (e.g. "build this way here") and long-term strategies guided by citywide, regional and even national visions (e.g. "imagine the benefits of making the city free from private cars by 2030") and policy drivers.
...
Transport planning should therefore adapt to contemporary policy priorities, which have shifted substantially over the past few decades.
If transport plans are developed based on out-of-date policies (such as accommodating growth in car ownership and use) or by unaccountable technocrats with little understanding of their impacts, the results can be severe.
the transport system now is the "leading cause of death for children and young adults aged 5-29 years" through road traffic casualties alone, with 1.35 million people killed and tens of millions injured and disabled each year [@world_health_organization_global_2018].
...
Transport planning has the potential to improve health outcomes by ensuring that motor traffic is constrained to areas where the fumes they emit can reach few lungs and, in the longer term, removing internal combustion engines from densley populated areas altogether.
Transport planning can also have positive health impacts by enabling millions of people to integrate physical activity into their everyday lives, thereby tackling one of the root causes of the 'obesity crisis': lack of regular physical activity [@yang_travelobesity_2013].
A wider issue global challenge in the 21^st^ Century is social and economic inequalities, which can be exacerbated or tackled by transport plans that can prioritise mobility for high income citizens or more socially equitable transport options that can be used by a wide range of people [@moreno-monroy_public_2017], from '8 to 80 year olds'.^[The term "from 8 to 80 year olds" is used informally by transport planning practitioners to convey their intention to design for everyone, as exemplified by the use of the phrase in a 2015 article in the [Guardian](https://www.theguardian.com/uk-news/davehillblog/2015/sep/23/whitechapel-high-street-cycle-superhighway-bus-stop-bypass-is-a-mess).]  
A global and perhaps more important recent policy driver of transport planning is climate change mitigation.
...
the statistical programming language R [@r_core_team_r:_2019],
the general purpose programming language Python [@rossum_python_1995],
and the desktop GIS QGIS [@qgis_development_team_qgis_2019].
...
Except for specific national transport policies such as fuel tax, transport policies tend to have geographic outcomes (to build new cycle infrastructure, for example, which must go somewhere).
...
Furthermore, in the age of evidence-based policy, open data and citizen science, there is an onus on practitioners to provide solutions that are transparent, accessible and, increasingly, participatory  [@banister_sustainable_2008].
...
A shift to open source planning tools in general, and open source tools for local planning in particular, can help help tackle wider problems, including the 'crisis of participatory planning' [@Legacy_2016], feelings of disempowerment due to lack of opportunities to engage in democratic processes --- which some have blamed for the election of populist leaders and the UK Brexit vote [@monbiot_out_2017] --- and low levels of data and software literacy [@Christozov_2017].
